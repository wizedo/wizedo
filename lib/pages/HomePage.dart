...
import '../controller/dependecy_injection.dart';
...
class _HomePageState extends State<HomePage>  {
  ...
  Future<void> _initialize() async {
    ...
    adLoaded();
    debugLog('below is height we got using getx');
    // debugLog(Get.height);
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>> buildDocumentStream(String postId) {
    debugLog('$postId post is updated');
  ...
  late RxBool isInterstitialLoaded = false.obs;
      ...
      return GestureDetector(
        onTap: () async {
          if (postsOpenedCount.value > 4) {
            debugLog('can show ad in homepage');
            // Check if the interstitial ad is loaded and if 60 seconds have passed since the ad was loaded
            // if (isInterstitialLoaded.value == true) {
            //   debugLog('isInterstitialLoaded is true');
            //   final timeElapsedhomepage = DateTime.now().difference(adLoadedTime!);
            //   debugLog('time elasped is: ${timeElapsedhomepage.inSeconds}');
            //   if (timeElapsedhomepage.inSeconds >= 120) {
            //     debugLog('time is greater than 120');
            ...
            //     adLoaded();
            //   } else {
            //     debugLog('Interstitial ad loaded but not ready to be shown yet');
            //   }
            // }
            if (isInterstitialLoaded.value == true) {
              ...
              debugLog('Interstitial ad not loaded');
            }
          }
  ...
  @override
  void dispose() {
    debugLog('widget and streams disposed');
  ...
  Future<void> fetchMoreDocuments(String category) async {
    try {
      ...
      if (newSnapshot.docs.isNotEmpty) {
        ...
        debugLog('No more documents for category: $category');
      ...
      debugLog('Error fetching more documents for category $category: $error');
    } finally {
  ...
  StreamBuilder<DocumentSnapshot<Map<String, dynamic>>> buildStreamBuilder(String postId) {
    debugLog('Building stream for post ID but not fetching or creating stream: $postId');
  ...
  Widget buildListViewForCategory(String category) {
    debugLog('building list view category for $category');
  ...
  StreamBuilder<QuerySnapshot<Map<String, dynamic>>> buildStreamBuilderForCategory(String category) {
    debugLog('Building stream for category: $category');
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
            if (_categoryDocuments[category] == null) {
              debugLog('Initially it was null, so if statement');
              _categoryDocuments[category] = snapshot.data!.docs;
              _lastDocument = _categoryDocuments[category]!.last;
            }
            debugLog('Data received. Number of documents: ${snapshot.data!.docs.length}');
            debugLog('Below is item count');
            debugLog(_categoryDocuments[category]!.length);
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
                          ...
                          children: [
                            ...
                            InkWell(
                              onTap: () {
                                debugLog('refresh');
                                _categoryDocuments.refresh();
                                debugLog('Searched Post IDs for College Project: $searchedPostIds');
                                debugLog('belwo is number of fetched search resulsts');
                                debugLog(searchResultsCount);
                              },
                              child: WhiteText(
                                      ...
                                      child: FloatingActionButton(
                                        ...
                                        onPressed: () {
                                          if (_auth.currentUser != null) {
                                            ...
                                            debugLog('Avatar: ${userController.avatar}');
                                            debugLog('ID: ${userController.id}');
                                            debugLog('First Name: ${userController.firstname}');
                                            debugLog('Last Name: ${userController.lastname}');
                                            debugLog('Country: ${userController.country}');
                                            debugLog('College: ${userController.college}');
                                            debugLog('Course: ${userController.course}');
                                            debugLog('Course Start Year: ${userController.courseStartYear}');
                                            debugLog('User Details Filled: ${userController.userDetailsfilled}');
                                            debugLog('Last Updated: ${userController.lastUpdated}');
                                            debugLog('Email Verified: ${userController.emailVerified}');
                                          } else {
            ...
            // buildStreamBuilderForCategory(_selectedCategory)
            Expanded(
              child: GestureDetector(
                ...
                child: Obx(() {
                  ...
                  return buildStreamBuilderForCategory(_selectedCategory);
                }),
              ),
            )
          ],

...
class OtpScreen extends StatefulWidget {
  ...
  const OtpScreen({super.key, required this.phoneNumber, required this.otp, this.onOtpVerified});
...
class _OtpScreenState extends State<OtpScreen> {
  ...
  Future<String?> getUserOtpLocally(String phoneNumber) async {
    try {
      ...
      debugLog('Error getting user OTP locally: $error');
      return null;
    }
  }

  Future<void> debugLogUserOtp(String phoneNumber) async {
    try {
      String? storedOtp = await getUserOtpLocally(phoneNumber);
      if (storedOtp != null) {
        debugLog('Stored OTP for $phoneNumber: $storedOtp');
      } else {
        debugLog('No stored OTP found for $phoneNumber');
      }
    } catch (error) {
      debugLog('Error debugLoging user OTP: $error');
    }
  }
  ...
  void verifyOtp() async {
    ...
    if (enteredOtp == widget.otp) {
      debugLog('OTP Verified. Navigating to next page.');
            ...
            child: Column(
              children: [
                Align(
                  ...
                  child: Row(
                    children: [
                      const WhiteText('Enter the OTP sent to +91 '),
                      WhiteText(
                        widget.phoneNumber,
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const WhiteText("Didn't receive OTP?", fontSize: 12),
                    MyTextButton(
                      onPressed: () {
                        Get.back();
                      },
...
class OtpEditText extends StatelessWidget {
  ...
  const OtpEditText({super.key, this.focusNode, this.controller, this.nextFocusNode});

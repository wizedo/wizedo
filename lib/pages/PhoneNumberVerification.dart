...
class _PhoneNumberVerificationState extends State<PhoneNumberVerification> {
  ...
  void navigateToPhoneNumberVerification() {
    debugLog("navigate to phoneverification executed");
  ...
  void navigateBackAfterDelay() {
    debugLog("navigate back to previouspage after 5 seconds and 3 wrong attempt");
  ...
  void _sendOTP(String phoneNumber) async {
    try {
      const url = "http://192.168.1.105:3000/send-otp"; // Update the URL with your server address
      ...
      if (response.statusCode == 200) {
        ...
        debugLog("Failed to send OTP request to server. Status code: ${response.statusCode}");
    ...
    Get.to(() => OtpScreen(
      ...
      onOtpVerified: (bool isVerified) async {
        if (isVerified) {
          ...
          _otpAttempts++;
          debugLog("otp attempts$_otpAttempts");
          if (_otpAttempts >= 3) {
